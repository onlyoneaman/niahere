...
- **Slack:** Thinking emoji reaction while processing. Thread awareness (auto-listens without @mention). Thread context (50 messages + attachments) fetched via `conversations.replies`. `[NO_REPLY]` sentinel for silent thread judgement. Watch mode: per-channel proactive monitoring via `channels.slack.watch` config — receives all messages, behavior prompt injected, uses per-thread sessions. File attachments cached to disk at `~/.niahere/tmp/attachments/`.
