...
- **Alive monitor:** (`src/core/alive.ts`) 60s heartbeat checks DB connectivity. On failure: attempts reconnect. After 30 consecutive failures (~30 min): triggers LLM recovery agent to diagnose/fix, then notifies user with postmortem. Falls back to direct channel notification if agent fails.
