...
- **Memory is disclosed, rules are loaded:** `loadIdentity()` loads `identity/owner/soul/rules` whole, then only part of `memory.md` — the curated undated sections plus the newest `## Promoted <date>` ones, up to `MEMORY_BUDGET_BYTES` (`src/utils/memory-window.ts`). The rest is reachable via the `search_memory` tool, and the prompt states how many entries were withheld so the model knows to look. Never apply this to `rules.md`: rules are verbs and an unloaded rule is not followed. Search is a plain scan on purpose — at this file size an index costs sync and can suggest a near-match where an exact scan abstains.
- **Secrets never reach durable memory:** `src/utils/secrets.ts` is the veto, applied at both write paths into the persona files (`addMemory()`, `addRule()`). `memory.md` and `rules.md` load into every session's prompt, so a credential there has permanent maximum-exposure blast radius. Match on the issuer's format (`sk-ant-api\d\d-`, `xox[abpsr]-`, `gh[pousr]_`, `AKIA…`), never on words like "key" or "token" — the agent must stay able to *discuss* credentials. Any new durable write path gets this guard too.
- **Delivery status:** only channels that hand a message outside this process (`slack`, `telegram`, `sms`, `whatsapp`, `phone`) start a reply as `pending`; `terminal`/`system` record `sent` immediately because there is nothing to confirm. An unknown channel is assumed to deliver — a false alarm is cheaper than a lost message. The `delivery` health check reports replies that never confirmed and deliberately never retries them: the row proves a send was started, not that it failed, so a retry double-sends whenever the crash happened after the API call.
...
- **Superseded replies fold forward:** coalescing decides which messages share a turn, but the message that obsoletes a reply arrives while the turn producing it is still running, so the stale answer went out first — on the mini, "i meant browser" sat 49 seconds behind an answer to the question it replaced. The turn callback receives a `TurnControl`; `decideDelivery()` (`common/reply.ts`) asks `superseded()` only when there is something to post, and a reply with a newer message already queued is held back rather than sent. Nothing is discarded: the text stays in the session, and the next turn's merged prompt carries a note that it never reached the reader, so one reply answers both. Silent turns never ask, so the watch channel's many no-reply turns cannot exhaust the `maxDeferrals` cap (2) that guarantees a busy room still gets an answer.
...
- **Codex refreshes itself; Claude did not:** Nia spawns the real `codex` CLI per turn, which renews its own token on use — `auth.json` shows `last_refresh` moving and `access_token` valid days ahead. The Claude path spawns the SDK's *vendored* binary instead, which never refreshed the shared credentials file. Read codex expiry from `access_token`, never `id_token` (a short-lived OIDC token that lapses ~1h after every refresh and would report lapsed on every healthy install).
