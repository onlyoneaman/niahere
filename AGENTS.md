...
- **Structured outputs:** `AgentSessionContext.outputSchema` (and `EngineOptions.outputSchema`) takes a JSON Schema that both backends enforce — Claude via `outputFormat`, codex via `--output-schema` written to a temp file. The validated object arrives as `structured` on the result event and `SendResult`. Used by Slack watch judgements (`src/channels/common/watch-judgement.ts`); the `[NO_REPLY]` sentinel parse stays as the fallback for a backend that produces none. Prefer a schema over a sentinel whenever a turn answers a decision rather than writing prose.
