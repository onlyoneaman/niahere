...
- **Alive monitor:** (`src/core/alive.ts`) 60s heartbeat runs health checks. On failure: attempts reconnect (DB), runs LLM recovery agent, notifies user with postmortem. Falls back to direct channel notification if agent fails.
