...
- **Structured outputs across both backends** — `outputSchema` on a session is enforced by Claude via `outputFormat` and by codex via `--output-schema`, and the validated object arrives as `structured` on the result. Slack watch judgements use it: the decision is now a field rather than a `[NO_REPLY]` sentinel parsed out of prose, which had produced sentinel-plus-content 47 times. The sentinel path remains as a fallback for a backend that declines the schema.
