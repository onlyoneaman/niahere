...
- **A bold-wrapped `[NO_REPLY]` was logged as a confused model** — the sentinel check stripped backticks but not asterisks, so `**[NO_REPLY]**` was suppressed correctly yet warned about as "mixed with content". The watch decision now lives in `src/channels/common/watch-judgement.ts` with a differential test pinning it byte-for-byte against the logic it replaced.
...
- **Messages that arrive mid-turn are answered together** — measured over 30 days on the mini, ~24% of messages in the busiest watch channel and ~35% in `slack-tech` and the owner's DM land within ten seconds of the one before. One turn per message meant the agent answered the first without knowing the other two existed, then answered a question the sender had already moved past — three turns, three replies, three times the tokens. `createTurnPump` (`src/channels/common/coalesce.ts`) buffers messages that arrive while a turn is running and delivers them as a single follow-up turn, keeping each verbatim and in order. No timers: a message that could be answered immediately still is. Slack routes through it; the other channels are mechanical follow-ups.
...
- **Structured outputs across both backends** — `outputSchema` on a session is enforced by Claude via `outputFormat` and by codex via `--output-schema`, and the validated object arrives as `structured` on the result and on `SendResult`. No caller sets it yet: the engine persists `ev.text` as the reply, which under a schema is raw JSON, so enabling it on the Slack watch would have written `{"reply":null}` into the message history that failover later replays. The seam ships tested; the first consumer waits on giving the engine a human-readable content to store.
- **`auth` health check, and a failover message that names the cause** — Nia never said its Claude sign-in had lapsed; it just answered as codex. Sign-in state is now read from each provider's own credential store (Claude's `.credentials.json`, codex's `auth.json` id-token expiry) and reported. An expired *refresh* token fails and says to run `claude`; one nearing expiry warns. A merely lapsed *access* token is deliberately not a failure — it renews on demand and an idle daemon holds one every night — but when the chain has also stopped using that provider, the failover check names it: "primary has not served for 16d (codex covering) — claude auth: access token lapsed 16d ago". The alive monitor reports sign-in problems directly rather than routing them to the LLM recovery agent, which runs through the very chain that is failing.
